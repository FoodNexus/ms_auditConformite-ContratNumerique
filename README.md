...
Cloud & Découverte : Spring Cloud Netflix Eureka Client
Intelligence Artificielle : CNN Deep Learning
Qualité de code & Tests : JUnit 5, Mockito, JaCoCo (Couverture > 77%), SonarQube Scanner

⚙️ Architecture & Dépendances

Ce microservice dépend de l'écosystème global FoodNexus :

Eureka Server : Accessible sur http://localhost:8761/eureka/
Keycloak Server : Accessible sur http://localhost:8180/realms/foodnexus
Base de données MySQL : foodnexus_audit (port 3306)
...
Serveurs Eureka et Keycloak fonctionnels (si vous testez les endpoints sécurisés)
1. Cloner le repository
git clone https://github.com/votre-organisation/ms_auditConformite-ContratNumerique.git
cd ms_auditConformite-ContratNumerique
2. Configuration (application.properties)
Vérifiez ou modifiez le fichier src/main/resources/application.properties si nécessaire. La base de données foodnexus_audit sera créée automatiquement grâce au paramètre createDatabaseIfNotExist=true.

3. Compiler et lancer
mvn clean install
mvn spring-boot:run
Le service démarrera par défaut sur le port 8083.
...
POST /scan : Analyser une image avec l'IA et créer un dossier d'inspection. (Nécessite le rôle AUDITOR)
GET /auditor/{auditorId} : Récupérer toutes les inspections d'un auditeur spécifique.
PATCH /{id}/verdict : Mettre à jour manuellement le verdict sanitaire.
📄 Contrats Numériques (/api/contracts)
