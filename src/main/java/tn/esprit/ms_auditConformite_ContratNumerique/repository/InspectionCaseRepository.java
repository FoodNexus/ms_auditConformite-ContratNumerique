...
public interface InspectionCaseRepository
    ...
    @Query("SELECT i.sanitaryVerdict as label, COUNT(i) as value FROM InspectionCase i WHERE (:auditorId IS NULL OR i.auditorId = :auditorId) GROUP BY i.sanitaryVerdict")
    List<Map<String, Object>> countByVerdict(@Param("auditorId") Long auditorId);

    @Query("SELECT i.resolutionStatus as label, COUNT(i) as value FROM InspectionCase i WHERE (:auditorId IS NULL OR i.auditorId = :auditorId) GROUP BY i.resolutionStatus")
    List<Map<String, Object>> countByStatus(@Param("auditorId") Long auditorId);

    @Query("SELECT FUNCTION('MONTH', i.creationDate) as month, COUNT(i) as value FROM InspectionCase i WHERE (:auditorId IS NULL OR i.auditorId = :auditorId) GROUP BY FUNCTION('MONTH', i.creationDate) ORDER BY 1")
    List<Map<String, Object>> countByMonth(@Param("auditorId") Long auditorId);

    @Query("SELECT (SELECT COUNT(r) FROM RecyclingProducts r WHERE (:auditorId IS NULL OR r.inspectionCase.auditorId = :auditorId)) * 100.0 / (SELECT COUNT(i) FROM InspectionCase i WHERE (:auditorId IS NULL OR i.auditorId = :auditorId))")
